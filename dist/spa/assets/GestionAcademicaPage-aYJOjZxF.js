...
import { Q as O } from './QSelect-B2vk19AQ.js'
import { Q as ra } from './QSpinnerDots-Cc_75p2S.js'
import { Q as D } from './QChip-g_JHqrFQ.js'
import { Q as _e } from './QSpace-PwS5FTKD.js'
import { Q as H } from './QTooltip-xFvievHl.js'
import { Q as na } from './QBadge-BSFZG2oj.js'
import { Q as ne } from './QItemLabel-wP2Wn6Fb.js'
import { Q as oe, a as te } from './QItem-C23xClkh.js'
import { Q as ua } from './QList-BSXQR-9A.js'
import { Q as da, a as ca } from './QTable-HDg5LzBZ.js'
import { Q as ma } from './QPage-D-YrLEJA.js'
import { C as X } from './ClosePopup-srUPe2x8.js'
import { u as pa } from './sedes-xi7lRAei.js'
import { u as va } from './carreras-B-7e1Sez.js'
import { u as ga } from './docentes-XcRqWH7f.js'
import { u as ba, a as _a } from './horarios-HkW-cpQY.js'
import { u as fa } from './asignaturas-C0o3NZaO.js'
...
const fe = {
    getBloques(C = {}) {
      return U.get('/bloques', { params: C })
    },
    createBloque(C) {
      return U.post('/bloques', C)
    },
    updateBloque(C, E) {
      return U.put(`/bloques/${C}`, E)
    },
    deleteBloque(C) {
      return U.delete(`/bloques/${C}`)
    },
  },
  ha = la('bloques', () => {
    const C = h([]),
    ...
    async function ue(A = {}) {
      E.value = !0
      try {
        const n = await fe.getBloques(A)
        C.value = n.data
      } catch (n) {
        console.error('Error fetching bloques:', n)
      } finally {
        E.value = !1
      }
    }
    async function de(A) {
      E.value = !0
      try {
        const n = await fe.createBloque(A)
        return (C.value.push(n.data), n.data)
      } catch (n) {
        throw (console.error('Error creating bloque:', n), n)
      } finally {
        E.value = !1
      }
    }
    async function R(A, n) {
      E.value = !0
      try {
        const p = await fe.updateBloque(A, n),
          T = C.value.findIndex((Y) => Y.id === A)
        return (T !== -1 && (C.value[T] = p.data), p.data)
      } catch (p) {
        throw (console.error('Error updating bloque:', p), p)
      } finally {
        E.value = !1
      }
    }
    async function B(A) {
      E.value = !0
      try {
        ;(await fe.deleteBloque(A), (C.value = C.value.filter((n) => n.id !== A)))
      } catch (n) {
        throw (console.error('Error deleting bloque:', n), n)
      } finally {
        E.value = !1
      }
    }
    function se(A) {
      return C.value.filter((n) => n.sede_id == A)
    }
    return {
      bloques: C,
      loading: E,
  ...
  Ha = { key: 2, class: 'col-3' },
  ja = { key: 0, class: 'row items-center no-wrap q-gutter-xs' },
  Fa = ['title'],
  ...
  Hl = { class: 'col-12 col-md-7' },
  jl = { class: 'row items-center q-mb-sm' },
  ...
  so = {
    ...
    setup(C) {
      ...
      const A = h(!1),
        ...
        ie = h(!1),
        I = h([]),
        Ve = L(() => E.sedes.map((l) => ({ label: l.nombre, value: l.id }))),
        Oe = L(() =>
          p.value
            ? ue.carreras
                .filter(
                  (l) =>
                    l.sede_id == p.value || (l.sedes_ids && l.sedes_ids.includes(Number(p.value))),
                )
        ...
        Z = L(() => {
          let l = I.value
          if ((Y.value && (l = l.filter((e) => e.plan_estudios === Y.value)), ce.value)) {
            const e = ce.value.toLowerCase()
            l = l.filter((a) =>
              a.nombre.toLowerCase().includes(e) || a.codigo.toLowerCase().includes(e)
                ? !0
                : a.grupos?.some(
                    (i) =>
        ...
        Ie = L(() => Z.value.reduce((l, e) => l + (e.grupos?.length || 0), 0)),
        ze = L(() =>
          Z.value.reduce(
      ...
      function Ue(l) {
        ...
        ;(e.has(l) ? e.delete(l) : e.add(l), (j.value = e))
      }
      function Be() {
        ;((ie.value = !ie.value),
          ie.value ? (j.value = new Set(Z.value.map((l) => l.id))) : (j.value = new Set()))
      }
      function qe(l) {
      ...
      oa(async () => {
        ;(await Promise.all([
          ...
          (pe.value = ee.value))
        const l = E.sedes.find((e) => e.nombre?.toLowerCase().includes('cochabamba'))
        l && ((p.value = l.id), await F())
      })
      function Re() {
        ;((T.value = null), (I.value = []), p.value && F())
      }
      async function F() {
        if (!(!p.value && !T.value)) {
          A.value = !0
          try {
            ...
            ;(p.value && (l.sede_id = p.value), T.value && (l.carrera_id = T.value))
            const a = (await U.get('/grupos-flat', { params: l })).data.data || [],
              i = {}
            ;(p.value && (i.sede_id = p.value), T.value && (i.carrera_id = T.value))
            const u = await U.get('/horarios', { params: i }),
              y = {}
            u.data.forEach((q) => {
              ;(y[q.grupo_id] || (y[q.grupo_id] = []), y[q.grupo_id].push(q))
            })
            const S = a.map((q) => ({ ...q, horarios: y[q.id] || [] })),
              ae = {}
            ;(S.forEach((q) => {
              ;(ae[q.asignatura_id] ||
                (ae[q.asignatura_id] = {
                  id: q.asignatura_id,
                  nombre: q.asignatura_nombre,
                  codigo: q.asignatura_codigo,
                  plan_estudios: q.asignatura_plan || null,
                  grupos: [],
                }),
                ae[q.asignatura_id].grupos.push(q))
            }),
              (I.value = Object.values(ae).sort((q, aa) => q.nombre.localeCompare(aa.nombre))))
          } catch (l) {
      ...
      async function $e() {
        ;(await Promise.all([R.fetchAulas(), B.fetchBloques()]), await F())
      }
      const ee = L(() =>
          de.docentes.map((l) => ({
            id: l.id,
            nombre_completo:
              l.nombre_completo || `${l.nombre || ''} ${l.apellido_paterno || ''}`.trim(),
          })),
        ),
        pe = h([])
      function De(l, e) {
        e(() => {
          pe.value = l
            ? ee.value.filter((a) => a.nombre_completo.toLowerCase().includes(l.toLowerCase()))
            : ee.value
        })
      }
      const ye = L(() =>
        ...
        he = h([])
      function Qe(l, e) {
        const a = x.value._sedeId || p.value,
      ...
      const Ge = L(() =>
        ...
        Ce = L(() => (p.value ? B.bloques.filter((l) => l.sede_id == p.value) : B.bloques)),
        v = h({
          asignatura: !1,
          grupo: !1,
          horario: !1,
        ...
        He = L(() => {
          ...
          const l = xe.value
          return l === 'asignatura'
            ? `¿Eliminar la asignatura "${P.value.nombre}"? Se eliminarán también sus grupos y horarios.`
            : l === 'grupo'
              ? `¿Eliminar el grupo "${P.value.nombre}" (${P.value.asignatura_nombre || ''})? Se eliminarán también sus horarios.`
              : l === 'horario'
                ? `¿Eliminar el horario del ${P.value.dia} ${P.value.hora_inicio}–${P.value.hora_fin}?`
                : l === 'aula'
                  ? `¿Eliminar el aula "${P.value.nombre}"?`
                  : l === 'bloque'
                    ? `¿Eliminar el bloque "${P.value.nombre}"? Solo se puede si no tiene aulas.`
                    : ''
        })
      function J(l, e, a = {}) {
        l === 'asignatura'
          ...
          : l === 'grupo'
            ? ((pe.value = ee.value),
              ...
              (v.value.grupo = !0))
            : l === 'horario' &&
              ((he.value = ye.value.filter((i) => !p.value || i.sede_id == p.value)),
              (x.value = e
                ? {
                    id: e.id,
                    dia: e.dia,
      ...
      function je() {
        ;((N.value = Ce.value[0] || null), (v.value.aulas = !0))
      }
      async function Fe() {
        const l = d.value
        ...
        try {
          ...
          if (l.id) {
            ...
            const a = I.value.find((i) => i.id === l.id)
      ...
      async function Je() {
        ...
        try {
          ...
          if (l.id) {
            await U.put(`/grupos/${l.id}`, e)
            const a = I.value.find((i) => i.grupos?.some((u) => u.id === l.id))
            if (a) {
              ...
              i &&
                Object.assign(i, {
                  ...
                  docente_nombre: e.docente_id
                    ? ee.value.find((u) => u.id === e.docente_id)?.nombre_completo ||
                      i.docente_nombre
                    : null,
                })
            }
            ...
            const a = await U.post('/grupos', e),
              i = I.value.find((u) => u.id === l.asignatura_id)
            ;(i
              ? i.grupos.push({
                  ...a.data,
      ...
      async function We() {
        ...
        try {
          const e = {
            ...
            a = l.aula_id ? R.getAulaById(l.aula_id) : null
          if (l.id) {
            const i = await U.put(`/horarios/${l.id}`, e)
            ...
            const i = await U.post('/horarios', e)
            for (const u of I.value) {
              const y = u.grupos?.find((S) => S.id === l.grupo_id)
              if (y) {
      ...
      const N = h(null),
        Ke = L(() => (N.value ? R.aulas.filter((l) => l.bloque_id === N.value.id) : [])),
        Xe = [
      ...
      async function Ze() {
        ...
        try {
          ...
          ;(l.id
            ? (await R.updateAula(l.id, e),
              g.create({ type: 'positive', message: 'Aula actualizada' }))
            : (await R.createAula(e), g.create({ type: 'positive', message: 'Aula creada' })),
            (v.value.aula = !1))
        } catch (e) {
          g.create({
            type: 'negative',
      ...
      function re(l, e) {
        ;((xe.value = l), (P.value = e), (v.value.eliminar = !0))
      }
      async function ea() {
        n.value = !0
        try {
          ...
          if (l === 'asignatura')
            (await se.deleteAsignatura(e.id),
              (I.value = I.value.filter((a) => a.id !== e.id)),
              g.create({ type: 'positive', message: 'Asignatura eliminada' }))
          else if (l === 'grupo') {
            await U.delete(`/grupos/${e.id}`)
            for (const a of I.value) a.grupos && (a.grupos = a.grupos.filter((i) => i.id !== e.id))
            g.create({ type: 'positive', message: 'Grupo eliminado' })
          } else if (l === 'horario') {
            await U.delete(`/horarios/${e.id}`)
          ...
          { class: 'q-pa-md gestion-page' },
          {
            default: t(() => [
              s('div', xa, [
                s('div', wa, [
                  s('div', Va, [
                    o(V, { name: 'grid_view', size: '28px', color: 'primary' }),
                    e[47] ||
                      (e[47] = s(
                        'div',
                        null,
                          ...
                          s('div', Ca, [
                            s('div', Aa, [
                              o(
                                ...
                                {
                                  prepend: t(() => [
                                    o(V, { name: 'location_city', size: '18px', color: 'grey-6' }),
                                  ]),
                                  _: 1,
                                },
                                8,
                            ...
                            s('div', Ea, [
                              o(
                                ...
                                {
                                  modelValue: T.value,
                                  'onUpdate:modelValue': [e[2] || (e[2] = (a) => (T.value = a)), F],
                                  options: Oe.value,
                                  label: 'Carrera',
                                  outlined: '',
                                  dense: '',
                                ...
                                {
                                  prepend: t(() => [
                                    o(V, { name: 'school', size: '18px', color: 'grey-6' }),
                                  ]),
                                  _: 1,
                                },
                                8,
                            ...
                            s('div', Na, [
                              o(
                                ...
                                {
                                  prepend: t(() => [
                                    o(V, { name: 'account_tree', size: '18px', color: 'grey-6' }),
                                  ]),
                                  _: 1,
                                },
                                8,
                            ...
                            s('div', Sa, [
                              o(
                                ...
                                {
                                  modelValue: ce.value,
                                  'onUpdate:modelValue': e[4] || (e[4] = (a) => (ce.value = a)),
                                  outlined: '',
                                  dense: '',
                                  clearable: '',
                                ...
                                {
                                  prepend: t(() => [
                                    o(V, { name: 'search', size: '18px', color: 'grey-6' }),
                                  ]),
                                  _: 1,
                                },
                                8,
              ...
              A.value
                ...
                : Z.value.length === 0
                  ? (r(),
                    b('div', Oa, [
                      o(V, { name: 'school', size: '56px', color: 'grey-4' }),
                      s(
                        'div',
                        Ia,
                        c(
                    ...
                    b('div', za, [
                      s('div', Ua, [
                        o(
                          ...
                          { default: t(() => [f(c(Z.value.length) + ' asignaturas ', 1)]), _: 1 },
                        ),
                        o(
                          D,
                          { size: 'sm', color: 'teal', 'text-color': 'white', dense: '' },
                          { default: t(() => [f(c(Ie.value) + ' grupos ', 1)]), _: 1 },
                        ),
                        o(
                          D,
                          { size: 'sm', color: 'orange-7', 'text-color': 'white', dense: '' },
                          { default: t(() => [f(c(ze.value) + ' horarios ', 1)]), _: 1 },
                        ),
                        o(_e),
                        o(
                          m,
                              ...
                              s(
                                'div',
                                {
                                  class: Ne([
                                    ...
                                    j.value.has(a.id) ? 'asig-header--open' : '',
                                  ]),
                                  onClick: (i) => Ue(a.id),
                                },
                                [
                                  o(
                                    V,
                                    {
                                      name: j.value.has(a.id) ? 'expand_more' : 'chevron_right',
                                      size: '20px',
                                      color: 'grey-6',
                                      class: 'q-mr-xs',
                                    },
                                ...
                                s('div', ql, [
                                  ...
                                  s('div', Sl, [
                                    o(
                                      ...
                                      {
                                        prepend: t(() => [o(V, { name: 'person', size: '18px' })]),
                                        option: t((a) => [
                                          o(
                                            oe,
                                            Se(ke(a.itemProps)),
                          ...
                          o(
                            ...
                            { class: 'bg-orange-8 text-white row items-center q-py-sm' },
                            {
                              default: t(() => [
                                o(
                                  V,
                                  { name: x.value.id ? 'edit' : 'add', class: 'q-mr-sm' },
                                  null,
                                  8,
                                  ['name'],
                                ),
                                s(
                                  'span',
                                  zl,
                                  c(x.value.id ? 'Editar Horario' : 'Nuevo Horario'),
                                  1,
                                ),
                                s(
                                  'span',
                                  Ul,
                                  ' — ' +
                                    c(x.value._asignaturaNombre) +
                                    ' / Grupo ' +
                                    c(x.value._grupoNombre),
                                  1,
                                ),
                              ]),
                              _: 1,
                                ...
                                s('div', Bl, [
                                  s('div', Tl, [
                                    o(
                                      O,
                                      {
                                        modelValue: x.value.dia,
                                        'onUpdate:modelValue':
                                          e[29] || (e[29] = (a) => (x.value.dia = a)),
                                        options: [
                                  ...
                                  s('div', Pl, [
                                    o(
                                      w,
                                      {
                                        modelValue: x.value.hora_inicio,
                                        'onUpdate:modelValue':
                                          e[30] || (e[30] = (a) => (x.value.hora_inicio = a)),
                                        label: 'Hora inicio *',
                                        type: 'time',
                                        outlined: '',
                                        dense: '',
                                  ...
                                  s('div', Ll, [
                                    o(
                                      w,
                                      {
                                        modelValue: x.value.hora_fin,
                                        'onUpdate:modelValue':
                                          e[31] || (e[31] = (a) => (x.value.hora_fin = a)),
                                        label: 'Hora fin *',
                                        type: 'time',
                                        outlined: '',
                                        dense: '',
                                  ...
                                  s('div', Ml, [
                                    o(
                                      O,
                                      {
                                        modelValue: x.value.aula_id,
                                        'onUpdate:modelValue':
                                          e[32] || (e[32] = (a) => (x.value.aula_id = a)),
                                        options: he.value,
                                  ...
                                  s('div', Rl, [
                                    o(
                                      w,
                                      {
                                        modelValue: x.value.id_horario_api,
                                        'onUpdate:modelValue':
                                          e[33] || (e[33] = (a) => (x.value.id_horario_api = a)),
                                        modelModifiers: { number: !0 },
                                        label: 'ID API externo',
                                        type: 'number',
                                        outlined: '',
                            ...
                            { align: 'right', class: 'q-pa-md' },
                            {
                              default: t(() => [
                                ...
                                o(
                                  m,
                                  {
                                    color: x.value.id ? 'orange-8' : 'positive',
                                    icon: x.value.id ? 'save' : 'add',
                                    label: x.value.id ? 'Guardar cambios' : 'Crear Horario',
                                    loading: n.value,
                                    onClick: We,
                                  },
                                  null,
                      ...
                      { style: { 'max-width': '1000px', width: '100%' } },
                      {
                        default: t(() => [
                          o(
                            ...
                            {
                              default: t(() => [
                                o(V, { name: 'meeting_room', class: 'q-mr-sm' }),
                                e[60] ||
                                  (e[60] = s(
                                    'span',
                      ...
                      { style: { width: '400px', 'max-width': '96vw' } },
                      {
                        default: t(() => [
                          o(
                            ...
                            {
                              default: t(() => [
                                s('span', Kl, c(Q.value.id ? 'Editar Bloque' : 'Nuevo Bloque'), 1),
                              ]),
                              _: 1,
                            },
                          ),
                              ...
                              default: t(() => [
                                s('div', Xl, [
                                  o(
                                    w,
                                    {
                                      modelValue: Q.value.nombre,
                                      'onUpdate:modelValue':
                                        e[38] || (e[38] = (a) => (Q.value.nombre = a)),
                                  ...
                                  o(
                                    O,
                                    {
                                      modelValue: Q.value.sede_id,
                                      'onUpdate:modelValue':
                                        e[39] || (e[39] = (a) => (Q.value.sede_id = a)),
                                      options: Ve.value,
                                      label: 'Sede *',
                                      outlined: '',
                                      dense: '',
                            ...
                            { align: 'right' },
                            {
                              default: t(() => [
                                ...
                                o(
                                  ...
                                  {
                                    color: 'grey-7',
                                    label: Q.value.id ? 'Guardar' : 'Crear',
                                    loading: n.value,
                                    onClick: Ye,
                                  },
                                  null,
                      ...
                      { style: { width: '420px', 'max-width': '96vw' } },
                      {
                        default: t(() => [
                          o(
                            ...
                            {
                              default: t(() => [
                                s('span', Yl, c(z.value.id ? 'Editar Aula' : 'Nueva Aula'), 1),
                                N.value
                                  ? (r(), b('span', Zl, ' — ' + c(N.value.nombre), 1))
                                  : $('', !0),
                              ]),
                              _: 1,
                            },
                              ...
                              default: t(() => [
                                s('div', eo, [
                                  o(
                                    w,
                                    {
                                      modelValue: z.value.nombre,
                                      'onUpdate:modelValue':
                                        e[41] || (e[41] = (a) => (z.value.nombre = a)),
                                  ...
                                  o(
                                    O,
                                    {
                                      modelValue: z.value.bloque_id,
                                      'onUpdate:modelValue':
                                        e[42] || (e[42] = (a) => (z.value.bloque_id = a)),
                                      options: Ge.value,
                                      label: 'Bloque *',
                                      outlined: '',
                                      dense: '',
                                  ...
                                  s('div', ao, [
                                    s('div', lo, [
                                      o(
                                        w,
                                        {
                                          modelValue: z.value.capacidad,
                                          'onUpdate:modelValue':
                                            e[43] || (e[43] = (a) => (z.value.capacidad = a)),
                                          modelModifiers: { number: !0 },
                                    ...
                                    s('div', oo, [
                                      o(
                                        w,
                                        {
                                          modelValue: z.value.pupitres,
                                          'onUpdate:modelValue':
                                            e[44] || (e[44] = (a) => (z.value.pupitres = a)),
                                          modelModifiers: { number: !0 },
                                          label: 'Pupitres',
                                          type: 'number',
                                          min: '0',
                            ...
                            { align: 'right' },
                            {
                              default: t(() => [
                                ...
                                o(
                                  ...
                                  {
                                    color: 'teal-8',
                                    label: z.value.id ? 'Guardar' : 'Crear',
                                    loading: n.value,
                                    onClick: Ze,
                                  },
                                  null,
  ...
  Io = ya(so, [['__scopeId', 'data-v-1bb3e22d']])
