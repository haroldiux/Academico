...
export const useProgramaAsignaturaStore = defineStore('programaAsignatura', () => {
  ...
  const horariosMateria = computed(() => {
    ...
    const materia = gruposStore.materiasExterno?.find(
      (m) => m.id === asignaturaId.value || m.codigo === asignaturaId.value,
    )
    ...
    try {
      ...
      if (response.data) {
        programa.value = {
          ...
          reglamento_normativa: response.data.reglamento_normativa || {
            clase: '',
            laboratorio: '',
          },
          sistema_evaluacion: response.data.sistema_evaluacion || {
            intro: '',
            diagnostica: '',
            formativa: '',
            sumativa: '',
