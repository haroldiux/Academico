...
export const useAsignaturasStore = defineStore(
  'asignaturas',
  () => {
    ...
    function updateElementoCompetencia(asigId, unidadId, data) {
      console.log('updateElemento', asigId, unidadId, data)
    }
    ...
    async function deleteUnidad(id) {
      ...
      try {
        ...
        if (asignaturaActual.value && asignaturaActual.value.unidades) {
          asignaturaActual.value.unidades = asignaturaActual.value.unidades.filter(
            (u) => u.id !== id,
          )
        }
      } catch (err) {
    ...
    async function deleteBibliografia(asigId, biblioId) {
      ...
      try {
        ...
        if (asignaturaActual.value && asignaturaActual.value.id === asigId) {
          asignaturaActual.value.bibliografias = asignaturaActual.value.bibliografias.filter(
            (b) => b.id !== biblioId,
          )
        }
      } catch (err) {
    ...
    function calcularProgresoTema(tema) {
      ...
      if (isDebug) {
        console.log(
          `[Progreso Debug] Tema ID: ${tema.id} | planificacion_personal:`,
          tema.planificacion_personal,
      ...
      const procedimental =
        getAny(tema, ['contenidos.procedimental', 'contenido_procedimental']) || []
      ...
      if (recursosEst.length > 1)
        recursosEst.slice(1).forEach((rec) => {
          totalCamposEst++
          if (rec?.trim()) camposLlenosEst++
        })
      ...
      secuencia.forEach((momento) => {
        if (momento.actividad?.trim()) camposLlenosSec++
      })
