...
export const usePreguntasStore = defineStore('preguntas', () => {
  ...
  function getEstadisticasByLogro(logroId) {
    ...
    return {
      ...
      porTipo: {
        seleccionMultiple: preguntasLogro.filter(
          (p) => p.tipo === TIPOS_PREGUNTA.SELECCION_MULTIPLE,
        ).length,
        seleccionUnica: preguntasLogro.filter((p) => p.tipo === TIPOS_PREGUNTA.SELECCION_UNICA)
          .length,
        falsoVerdadero: preguntasLogro.filter((p) => p.tipo === TIPOS_PREGUNTA.FALSO_VERDADERO)
          .length,
      },
      porDificultad: {
