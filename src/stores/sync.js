            ...
            for (const item of toSync) {
                try {
                    ...
                    if (item.data.evidencias_offline) {
                        ...
                        if (evtApr) formData.append('evidencia_aprendizaje', evtApr, item.data.evidencias_offline.aprendizaje_activo.name)

                        const evtEval = await getFileBlob(item.data.evidencias_offline.evaluacion_formativa)
                        if (evtEval) formData.append('evidencia_evaluacion', evtEval, item.data.evidencias_offline.evaluacion_formativa.name)

                        const evtSec = await getFileBlob(item.data.evidencias_offline.secuencia_didactica)
                        if (evtSec) formData.append('evidencia_secuencia', evtSec, item.data.evidencias_offline.secuencia_didactica.name)
                    }

                    if (item.data.integracion_transversal) {
                        const evtInv = await getFileBlob(item.data.integracion_transversal.investigacion?.evidencia)
                        if (evtInv) formData.append('evidencia_investigacion', evtInv, item.data.integracion_transversal.investigacion.evidencia.name)

                        const evtInt = await getFileBlob(item.data.integracion_transversal.interaccion?.evidencia)
                        if (evtInt) formData.append('evidencia_interaccion', evtInt, item.data.integracion_transversal.interaccion.evidencia.name)

                        const evtIntern = await getFileBlob(item.data.integracion_transversal.internalizacion?.evidencia)
                        if (evtIntern) formData.append('evidencia_internalizacion', evtIntern, item.data.integracion_transversal.internalizacion.evidencia.name)
                    }
                    ...
                    const deleteFile = async (fileObj) => {
                        if (fileObj && fileObj.path) {
                            try { await Filesystem.deleteFile({ path: fileObj.path, directory: Directory.Data }); } catch { /* ignore */ }
                        }
                    }
                    if (item.data.evidencias_offline) {
