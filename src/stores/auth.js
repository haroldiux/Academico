  ...
  () => {
    ...
    async function login(username, password) {
      try {
        ...
        if (!response.data.user) {
          throw new Error(
            'API returned 200 but user is missing: ' +
        ...
        usuarioActual.value = {
            ...
            for (const g of gruposRaw) {
              ...
              if (!grouped[groupKey]) {
                grouped[groupKey] = {
                  ...
                  estadisticas: asig.estadisticas_progreso || {
                    total: 0,
                    completados: 0,
                    pendientes: 0,
                  },
          ...
          carreras: [
            ...new Set(
              user.docente?.asignaturas?.flatMap((a) => a.carreras?.map((c) => c.nombre)) || [],
            ),
          ],
          // Fix: Load groups from docente.grupos relation, fallback to legacy
          grupos:
            user.docente?.grupos ||
            user.docente?.asignaturas
              ?.map((a) => a.pivot?.grupo)
    ...
    function tieneAccesoCarrera(carreraIdObjetivo, sedeIdObjetivo) {
      ...
      if (alcanceActual === 'carrera') {
        return (
          usuarioActual.value.carrera_id === carreraIdObjetivo &&
    ...
    function tieneAccesoMateria(materiaId, carreraIdMateria, sedeIdMateria) {
      ...
      if (alcanceActual === 'carrera') {
        return (
          usuarioActual.value.carrera_id === carreraIdMateria &&
