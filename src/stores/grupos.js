...
export const useGruposStore = defineStore(
  'grupos',
  () => {
    const materias = ref([]) // List of subjects with nested groups
    const materiasSincronizadas = ref([]) // Datos sincronizados permanentemente desde backend local
    ...
    // Computed: Estadísticas de materias sincronizadas
    const statsExterno = computed(() => ({
      totalMaterias: materiasSincronizadas.value.length,
      totalGrupos: materiasSincronizadas.value.reduce((sum, m) => sum + (m.grupos?.length || 0), 0),
    ...
    // Actions - API Externa
    async function fetchGruposSincronizados(params = {}) {
      loading.value = true
      error.value = null
      try {
        const response = await gruposExternoService.listarGrupos(params)
        materiasSincronizadas.value = response.data.data || []
    ...
    async function refrescarGruposSincronizados(params = {}) {
      loading.value = true
      try {
    ...
    return {
      ...
      // Actions
      fetchGrupos,
      fetchGruposSincronizados,
      refrescarGruposSincronizados,
      createGrupo,
    }
  },
  {
