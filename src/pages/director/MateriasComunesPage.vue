    ...
    <q-dialog v-model="dialogLink" persistent>
      ...
      >
        ...
        <q-card-section class="q-pa-md">
          <div class="row q-col-gutter-md">
                  ...
                  <q-item v-bind="scope.itemProps" dense>
                    <q-item-section>
                      ...
                      <q-item-label caption
                        >{{ scope.opt.codigo }} · {{ scope.opt.carrera_nombre }}</q-item-label
                      >
                    </q-item-section>
                  </q-item>
                </template>
            ...
            <div class="col-12 col-md-7" v-if="materiaBase">
              ...
              <!-- Sin resultados -->
              <div
                v-else-if="!loadingCandidatos && candidatos.length === 0 && searchCandidatos"
        ...
        <q-card-actions
          align="right"
          class="q-pa-md"
          v-if="materiaBase && materiasSeleccionadas.length"
        >
          <div class="text-caption text-grey-7 q-mr-auto">
            Vincular <strong>{{ materiaBase.nombre }}</strong> con
            {{ materiasSeleccionadas.length }} materia(s)
          </div>
      ...
      <q-card style="min-width: 350px; max-width: 500px">
        ...
        <q-card-section v-if="grupoDetalle">
          ...
          <div class="text-caption text-grey-7 q-mb-xs">
            Vinculadas ({{ grupoDetalle.vinculadas?.length }}):
          </div>
          <q-list dense>
...
const filterMisAsignaturas = (val, update) => {
  if (val === '') {
    update(() => {
      misAsignaturasFiltered.value = misAsignaturas.value
    })
    return
  }
  update(() => {
    const needle = val.toLowerCase()
    misAsignaturasFiltered.value = misAsignaturas.value.filter(
      (v) => v.nombre.toLowerCase().includes(needle) || v.codigo.toLowerCase().includes(needle),
    )
  })
}
...
@media (max-width: 768px) {
  .dialog-link-card .q-card-section {
    padding: 12px;
  }
}
</style>
