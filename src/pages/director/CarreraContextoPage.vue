<template>
    <q-page class="contexto-page">
        ...
        <q-header elevated class="bg-white text-primary border-bottom">
            <q-toolbar class="q-py-md">
                ...
                <div class="carrera-selector" v-if="carrerasDisponibles.length > 1">
                    <q-select dense outlined v-model="carreraSeleccionada" :options="carrerasDisponibles"
                        option-label="nombre" option-value="id" label="Seleccionar Carrera" bg-color="grey-1"
                        style="min-width: 250px" @update:model-value="cambiarCarrera">
                        <template v-slot:prepend>
                            <q-icon name="school" color="primary" />
                        </template>
                    </q-select>
                </div>
                <div v-else-if="carreraSeleccionada"
                ...
                <div class="col-12">
                    <q-card class="section-card">
                        <q-card-section>
                            ...
                            <q-input v-model="form.area" outlined dense
                ...
                <div class="col-12">
                    <q-card class="section-card">
                        <q-card-section>
                            ...
                            <q-editor v-model="form.mision" min-height="150px" :toolbar="[
                ...
                <div class="col-12">
                    <q-card class="section-card">
                        <q-card-section>
                            ...
                            <q-editor v-model="form.vision" min-height="150px" :toolbar="[
                                ['bold', 'italic', 'underline'],
                                ['unordered', 'ordered'],
                                ['undo', 'redo']
                ...
                <div class="col-12">
                    <q-card class="section-card">
                        <q-card-section>
                            ...
                            <q-editor v-model="form.perfil_profesional" min-height="300px" :toolbar="[
                                ['bold', 'italic', 'underline'],
                                ['unordered', 'ordered'],
                                ['undo', 'redo'],
                                ['viewsource']
        ...
        <q-page-sticky position="bottom-right" :offset="[24, 24]">
            <q-btn fab icon="save" label="Guardar Cambios" color="primary" :loading="saving" @click="guardarContexto"
                class="shadow-4" />
        </q-page-sticky>
    </q-page>
</template>
...
async function guardarContexto() {
    ...
    try {
        ...
        $q.notify({ type: 'positive', message: 'Información actualizada correctamente', icon: 'check_circle' })
    } catch (error) {
        console.error(error)
