      ...
      >
        <q-card flat bordered class="docente-card full-height">
              ...
              <div class="col overflow-hidden">
                ...
                <div class="row q-gutter-xs q-mt-xs">
                  ...
                  <q-badge
                    color="blue-1"
                    text-color="blue-8"
                    :label="docente.tipo_dedicacion || 'TIEMPO_PARCIAL'"
                  />
                </div>
              </div>
          ...
          <q-card-section class="q-pt-sm" style="min-height: 80px">
            ...
            <div
              v-if="docente.materiasData && docente.materiasData.length > 0"
              class="row q-gutter-xs"
            >
              <q-chip
                v-for="materia in docente.materiasData.slice(0, 3)"
                :key="materia.codigo"
                dense
                size="xs"
                color="purple-1"
            ...
            <div v-else class="text-caption text-grey-5 text-italic">Sin materias asignadas</div>
          </q-card-section>
          ...
          <div class="row items-center q-mb-lg">
            ...
            <div>
              ...
              <div class="text-caption text-grey-6">
                {{ detalleSeleccionado.materia.codigo }} • Grupo
                {{ detalleSeleccionado.materia.grupo }} • Docente:
                {{ detalleSeleccionado.docente.nombre_completo }}
              </div>
            </div>
          </div>
          ...
          <q-list dense bordered class="rounded-borders">
            <q-item>
              <q-item-section avatar>
                <q-icon
                  :name="detalleSeleccionado.materia.programaAnalitico ? 'check_circle' : 'cancel'"
                  :color="detalleSeleccionado.materia.programaAnalitico ? 'positive' : 'negative'"
                />
              </q-item-section>
              <q-item-section>
                <q-item-label>Programa Analítico</q-item-label>
              </q-item-section>
              <q-item-section side>
                <q-chip
                  :color="detalleSeleccionado.materia.programaAnalitico ? 'positive' : 'negative'"
                  ...
                  size="sm"
                >
                  {{ detalleSeleccionado.materia.programaAnalitico ? 'Entregado' : 'Pendiente' }}
                </q-chip>
              </q-item-section>
            </q-item>
            <q-item>
              <q-item-section avatar>
                <q-icon
                  :name="detalleSeleccionado.materia.pac ? 'check_circle' : 'cancel'"
                  :color="detalleSeleccionado.materia.pac ? 'positive' : 'negative'"
                />
              </q-item-section>
              <q-item-section>
                <q-item-label>Programa de Asignatura (PAC)</q-item-label>
              </q-item-section>
              <q-item-section side>
                <q-chip
                  :color="detalleSeleccionado.materia.pac ? 'positive' : 'negative'"
            ...
            <q-item>
              <q-item-section avatar>
                <q-icon
                  :name="detalleSeleccionado.materia.planClase ? 'check_circle' : 'cancel'"
            ...
            <q-item>
              <q-item-section avatar>
                <q-icon
                  :name="detalleSeleccionado.materia.cronograma ? 'check_circle' : 'cancel'"
                  :color="detalleSeleccionado.materia.cronograma ? 'positive' : 'negative'"
                />
              </q-item-section>
              <q-item-section>
                <q-item-label>Cronograma</q-item-label>
              </q-item-section>
              <q-item-section side>
                <q-chip
                  :color="detalleSeleccionado.materia.cronograma ? 'positive' : 'negative'"
                  ...
                  size="sm"
                >
                  {{ detalleSeleccionado.materia.cronograma ? 'Entregado' : 'Pendiente' }}
                </q-chip>
              </q-item-section>
            </q-item>
          </q-list>
...
const descargarReporteDetalle = () => {
  if (detalleSeleccionado.value) {
    ...
    const content = [
      ...
      `Estado Cronograma;${m.cronograma ? 'Entregado' : 'Pendiente'}`,
      `Estado General;${m.estado}`,
      `Fecha Generación;${new Date().toLocaleString()}`,
    ].join('\r\n')

    const status = exportFile(`Reporte_${m.codigo}_${d.ci}.csv`, content, 'text/csv')
...
const exportarLista = () => {
  ...
  const rows = docentesFiltrados.value.map(
    (d) =>
  ...
  const status = exportFile('lista-docentes.csv', content, 'text/csv')
