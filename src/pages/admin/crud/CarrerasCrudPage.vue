  ...
  <q-page class="q-pa-lg">
    <div class="row items-center q-mb-lg">
      <div class="col">
        ...
        <p class="q-ma-none q-mt-xs" style="color: var(--text-secondary)">
          Crear, editar y eliminar carreras (solo SUPER_ADMIN)
        </p>
      </div>
      <div class="col-auto">
        <q-btn
    ...
    <q-dialog v-model="showDialog" persistent>
      <q-card style="width: 600px; max-width: 90vw;">
        ...
        <q-card-section class="q-pt-lg">
          <q-form @submit="saveCarrera" class="q-gutter-md">
    ...
    <q-dialog v-model="showDeleteDialog" persistent>
      <q-card>
        ...
        <q-card-section class="q-pt-lg">
          ¿Estás seguro de eliminar la carrera "{{ carreraToDelete?.nombre }}"?
          <br>
...
const columns = [
  ...
  { name: 'sede_id', label: 'Sede ID', field: 'sede_id', align: 'center' },
...
const carreras = computed(() => carrerasStore.carreras)
...
async function saveCarrera() {
  ...
  try {
    if (editing.value) {
      await carrerasStore.updateCarrera(form.value.id, form.value)
      Notify.create({
        type: 'positive',
        message: 'Carrera actualizada correctamente'
      })
    } else {
      await carrerasStore.createCarrera(form.value)
      Notify.create({
        type: 'positive',
...
function confirmDelete(carrera) {
  carreraToDelete.value = carrera
