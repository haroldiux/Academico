  ...
  <q-page class="q-pa-lg">
    ...
    <div class="row items-center q-mb-lg">
      ...
      <div class="col-auto q-gutter-sm">
        ...
        <q-btn color="primary" icon="add" label="Nueva Asignatura" @click="openDialog(null)" :disable="loading" />
      </div>
    </div>
      ...
      <q-card-section>
        <div class="row q-col-gutter-md items-end">
          <div class="col-12 col-md-3">
            <q-input v-model="filter" outlined dense clearable placeholder="Buscar por nombre o código...">
    ...
    <!-- Tabla -->
    <q-table
      :rows="asignaturas" :columns="columns" row-key="id"
      :loading="loading" :filter="filter" :pagination="pagination"
      class="shadow-1 asig-table" flat bordered
    >
      ...
      <template #body-cell-plan_estudios="props">
        <q-td :props="props">
          <q-chip v-if="props.value" size="sm" dense
            ...
            <q-icon :name="props.value === 'A' ? 'history' : 'new_releases'" size="14px" class="q-mr-xs" />
      ...
      <template #body-cell-estado="props">
        <q-td :props="props">
          <q-chip size="sm" dense
            :color="props.value === 'ACTIVO' ? 'green-2' : props.value === 'EN_PROCESO' ? 'blue-2' : 'grey-3'"
            :text-color="props.value === 'ACTIVO' ? 'green-9' : props.value === 'EN_PROCESO' ? 'blue-9' : 'grey-7'">
            {{ props.value || '—' }}
          </q-chip>
        </q-td>
      </template>
      ...
      <template #body-cell-horas="props">
        <q-td :props="props">
          <span class="text-caption">T:{{ props.row.horas_teoricas || 0 }} P:{{ props.row.horas_practicas || 0 }}</span>
        </q-td>
      </template>
            ...
            <q-form @submit.prevent="saveAsignatura">
                ...
                <div class="row q-col-gutter-md">
                  <div class="col-12 col-md-3">
                    <q-input v-model="form.codigo" label="Código *" outlined dense
                      :disable="editing" hint="No editable una vez creado"
              ...
              <div class="form-section q-mb-lg">
                ...
                <div class="row q-col-gutter-md">
                  ...
                  <div class="col-12 col-md-4">
                    <q-select v-model="form.modalidad"
                      :options="[
              ...
              <div class="form-section q-mb-lg">
                ...
                <div class="row q-col-gutter-md">
                  <div class="col-6 col-md-2">
                    <q-input v-model.number="form.creditos" label="Créditos"
                      type="number" min="0" outlined dense />
                  </div>
                  <div class="col-6 col-md-2">
                    <q-input v-model.number="form.carga_horaria_total" label="Horas Total"
                      type="number" min="0" outlined dense />
                  </div>
                  <div class="col-6 col-md-2">
                    <q-input v-model.number="form.horas_teoricas" label="Horas Teóricas"
                      type="number" min="0" outlined dense />
                  </div>
                  <div class="col-6 col-md-2">
                    <q-input v-model.number="form.horas_practicas" label="Horas Prácticas"
                      type="number" min="0" outlined dense />
                  </div>
                  <div class="col-6 col-md-2">
                    <q-input v-model.number="form.horas_laboratorio" label="Horas Lab."
                      type="number" min="0" outlined dense />
                  </div>
                  <div class="col-6 col-md-2">
                    <q-input v-model.number="form.sesiones_semanales" label="Sesiones/Semana"
                      type="number" min="0" outlined dense />
                  </div>
                  <div class="col-6 col-md-2">
                    <q-input v-model.number="form.sesiones_semanales_teoricas" label="Ses. Teóricas"
                      type="number" min="0" outlined dense />
                  </div>
                  <div class="col-6 col-md-2">
                    <q-input v-model.number="form.sesiones_semanales_practicas" label="Ses. Prácticas"
                      type="number" min="0" outlined dense />
                  </div>
                </div>
              </div>
              ...
              <div class="form-section q-mb-lg">
                ...
                <div class="row q-col-gutter-md">
                  <div class="col-12 col-md-4">
                    <q-input v-model="form.area_desempenio" label="Área de Desempeño"
                      outlined dense clearable />
                  </div>
                  <div class="col-12 col-md-4">
                    <q-input v-model="form.tipo_curso" label="Tipo de Curso"
                      outlined dense clearable
                      placeholder="Ej: Obligatorio, Electivo..." />
                  </div>
                  <div class="col-12 col-md-4">
                    <q-input v-model="form.requisitos" label="Requisitos (prerrequisitos)"
                      outlined dense clearable
      ...
      <q-card style="min-width:360px">
        ...
        <q-card-section class="q-pt-lg">
          <div v-if="asignaturaToDelete" class="q-pa-sm bg-grey-2 rounded-borders q-mb-sm">
            ...
            <div><strong>Malla:</strong> {{ asignaturaToDelete.plan_estudios === 'A' ? 'Antigua' : asignaturaToDelete.plan_estudios === 'N' ? 'Nueva' : 'Sin definir' }}</div>
          </div>
...
// ── Formulario ──
const form = ref({
  id: null, codigo: '', sigla: '', nombre: '',
  plan_estudios: null, estado: 'EN_PROCESO', modalidad: null,
  creditos: 0, carga_horaria_total: 0,
  horas_teoricas: 0, horas_practicas: 0, horas_laboratorio: 0,
  sesiones_semanales: 0, sesiones_semanales_teoricas: 0, sesiones_semanales_practicas: 0,
  area_desempenio: '', tipo_curso: '', requisitos: '',
...
async function saveAsignatura() {
  if (!form.value.codigo?.trim()) { Notify.create({ type: 'warning', message: 'Ingresa el código' }); return }
  if (!form.value.nombre?.trim()) { Notify.create({ type: 'warning', message: 'Ingresa el nombre' }); return }
...
// ── Eliminar ──
function confirmDelete(asig) { asignaturaToDelete.value = asig; showDeleteDialog.value = true }
...
.asig-table  { border-radius: 12px; overflow: hidden; }
.asig-table :deep(th) {
  font-weight: 600; font-size: .8rem; text-transform: uppercase; letter-spacing: .5px; color: var(--text-secondary);
}
.form-section { border: 1px solid var(--border-color, #e0e0e0); border-radius: 10px; padding: 14px 16px; background: #fafafa; }
.form-section-title { font-weight: 600; font-size: .9rem; border-bottom: 1px solid #eee; padding-bottom: 8px; margin-bottom: 12px; }
</style>
