    ...
    <div class="row items-center q-mb-lg">
      <div class="col">
        ...
        <p class="q-ma-none q-mt-xs text-grey-6">
          Edita directamente los datos locales — los cambios no se sincronizan con la API
          universitaria
        </p>
      </div>
    ...
    >
        ...
        <q-td :props="props">
          <div class="col">
            ...
            <div class="row q-gutter-xs q-mt-xs">
              ...
              <q-chip
                v-if="props.row.asignatura_plan"
                size="xs"
                dense
      ...
      <template #body-cell-plan_estudios="props">
        <q-td :props="props">
          ...
          <q-chip v-else size="sm" dense color="grey-3" text-color="grey-7"> Sin definir </q-chip>
        </q-td>
      </template>
      ...
      <template #body-cell-estado="props">
        <q-td :props="props">
          <q-chip
            ...
            dense
            :color="
              props.value === 'ACTIVO' ? 'green-2' : props.value === 'INACTIVO' ? 'red-2' : 'grey-3'
            "
            :text-color="
              props.value === 'ACTIVO' ? 'green-9' : props.value === 'INACTIVO' ? 'red-9' : 'grey-7'
            "
          >
            {{ props.value || '—' }}
          </q-chip>
            ...
            <q-form @submit.prevent="saveGrupo">
                ...
                <q-card v-if="asigActual" flat bordered class="bg-blue-1 q-pa-sm">
                  <div class="row items-center q-gutter-sm no-wrap">
                    ...
                    <div class="col">
                      ...
                      <div class="row q-gutter-xs q-mt-xs">
                        <q-chip size="xs" dense outline color="grey-7">{{
                          asigActual.codigo
                        }}</q-chip>
                        <q-chip
                          v-if="asigActual.plan_estudios"
                          size="xs"
                          dense
              ...
              <div class="form-section q-mb-lg" :class="{ 'section-disabled': !asigActual }">
                ...
                <div class="row q-col-gutter-md">
                  ...
                  <div class="col-12 col-md-4">
                    <q-input
                      v-model.number="form.id_horario_api"
        ...
        <q-card-section class="q-pt-lg">
          <div v-if="grupoToDelete" class="q-pa-sm bg-grey-2 rounded-borders q-mb-sm">
            ...
            <div>
              <strong>Malla:</strong>
              {{
                grupoToDelete.plan_estudios === 'A'
                  ? 'Antigua'
                  : grupoToDelete.plan_estudios === 'N'
                    ? 'Nueva'
                    : 'Sin definir'
              }}
            </div>
...
const hasActiveFilters = computed(
  () => !!filterSede.value || !!filterCarrera.value || !!filterTexto.value || !!filterPlan.value,
)
...
async function loadData(page = 1) {
  ...
  try {
    ...
    pagination.value = {
      ...
      rowsNumber: resp.data.meta?.total || 0,
    }
  } catch (err) {
...
function onTableRequest(props) {
  ...
  loadData(props.pagination.page)
}
...
function cambiarAsignatura() {
  ...
  gruposExistentes.value = []
  busqAsig.value = ''
  resAsig.value = []
}

async function cargarGruposExistentes(asignaturaId) {
  loadingGruposExist.value = true
  gruposExistentes.value = []
  try {
    ...
    loadingGruposExist.value = false
  }
}
...
async function saveGrupo() {
  if (!form.value.asignatura_id) {
    Notify.create({ type: 'warning', message: 'Selecciona una asignatura' })
    return
  }
  if (!form.value.nombre?.trim()) {
    Notify.create({ type: 'warning', message: 'Ingresa el nombre del grupo (A, B, 1, 2...)' })
    return
  }
  if (!form.value.gestion?.trim()) {
    Notify.create({ type: 'warning', message: 'Ingresa la gestión (ej: 1-2026)' })
    return
  }
