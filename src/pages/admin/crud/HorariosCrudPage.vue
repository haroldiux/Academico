  ...
  <q-page class="q-pa-lg">
    ...
    <div class="row items-center q-mb-lg">
      ...
      <div class="col-auto q-gutter-sm">
        <q-btn outline color="primary" icon="refresh" label="Actualizar" @click="loadData" :loading="loading" />
        <q-btn color="primary" icon="add" label="Nuevo Horario" @click="openDialog(null)" :disable="loading" />
      </div>
    </div>
    ...
    <q-card flat bordered class="q-mb-lg filter-card">
      <q-card-section>
        <div class="row items-center q-mb-sm">
          ...
          <q-space />
          <q-btn v-if="hasActiveFilters" flat dense size="sm" color="negative"
            label="Limpiar filtros" icon="clear_all" @click="clearFilters" />
        </div>
        <div class="row q-col-gutter-md">
          ...
          <div class="col-12 col-md-3">
            <q-select v-model="filterDia" :options="diasOptions" label="Día"
              outlined dense clearable emit-value map-options>
              <template #prepend><q-icon name="today" size="20px" /></template>
            </q-select>
          </div>
          <div class="col-12 col-md-3">
            <q-input v-model="filterTexto" outlined dense
              placeholder="Buscar asignatura, docente, grupo..." clearable>
    ...
    <!-- Tabla -->
    <q-table :rows="filteredHorarios" :columns="columns" row-key="id"
      :loading="loading" :pagination="pagination"
      class="shadow-1 horarios-table" flat bordered>
      ...
      <template #body-cell-aula="props">
        <q-td :props="props">
          <div v-if="props.row.aula" class="row items-center no-wrap q-gutter-xs">
            ...
            <q-badge v-if="props.row.aula.bloque" color="grey-4" text-color="grey-8" outline class="q-ml-xs">
      ...
      <template #body-cell-dia="props">
        <q-td :props="props">
          <q-chip :color="diaColor(props.value)" :text-color="diaTextColor(props.value)" size="sm" dense>
      ...
      <template #body-cell-modificado_localmente="props">
        <q-td :props="props">
          <q-chip :color="props.value ? 'orange-1' : 'green-1'"
            :text-color="props.value ? 'orange-9' : 'green-9'"
            :icon="props.value ? 'edit' : 'cloud_done'" size="sm" dense>
          ...
          <q-form ref="formRef" @submit.prevent="saveHorario">
            ...
            <div class="step-block q-mb-md">
              <div class="step-header row items-center q-mb-sm">
                ...
                <q-btn v-if="asignaturaActual" flat dense size="sm" color="primary"
                  icon="swap_horiz" label="Cambiar" @click="cambiarAsignatura" />
              </div>
              ...
              <q-card v-if="asignaturaActual" flat bordered class="bg-blue-1 q-pa-sm">
                <div class="row items-center no-wrap q-gutter-sm">
                  ...
                  <div>
                    ...
                    <div class="text-caption text-grey-7">Código: {{ asignaturaActual.codigo }}</div>
                  </div>
                </div>
              </q-card>
              ...
              <div v-else>
                ...
                <q-card v-if="resAsig.length" flat bordered class="q-mt-xs resultado-list">
                  <q-list separator>
                    <q-item v-for="a in resAsig" :key="a.id" clickable v-ripple @click="elegirAsignatura(a)">
                      <q-item-section>
            ...
            <div class="step-block q-mb-md" :class="{ 'step-disabled': !asignaturaActual }">
              <div class="step-header row items-center q-mb-sm">
                <q-chip :color="asignaturaActual ? 'teal' : 'grey-4'"
                  :text-color="asignaturaActual ? 'white' : 'grey-6'"
                  size="sm" class="q-mr-sm">2</q-chip>
                <span class="text-weight-bold" :class="{ 'text-grey-5': !asignaturaActual }">
                  Grupo
                </span>
                <q-space />
                <!-- Botón "Cambiar" cuando ya hay grupo seleccionado -->
                <q-btn v-if="grupoActual && asignaturaActual" flat dense size="sm"
                  color="teal" icon="swap_horiz" label="Cambiar" @click="cambiarGrupo" />
              </div>
                ...
                <div v-else>
                  ...
                  <template v-else>
                    <q-card v-if="gruposAsignatura.length" flat bordered class="resultado-list">
                      <q-list separator>
                        <q-item v-for="g in gruposAsignatura" :key="g.id" clickable v-ripple @click="elegirGrupo(g)">
                          <q-item-section avatar>
                            <q-avatar :color="g.tipo === 'TEORICO' ? 'blue-2' : 'green-2'" size="36px">
                              <span :class="g.tipo === 'TEORICO' ? 'text-blue-9' : 'text-green-9'"
                                class="text-weight-bold text-caption">{{ g.nombre }}</span>
                            </q-avatar>
                          </q-item-section>
                          <q-item-section>
                            <q-item-label>
                              <span class="text-weight-bold">Grupo {{ g.nombre }}</span>
                              <q-chip size="xs" dense outline
                                :color="g.tipo === 'TEORICO' ? 'blue' : 'green'" class="q-ml-xs">
                                {{ g.tipo }}
                              </q-chip>
                            </q-item-label>
            ...
            <div class="step-block q-mb-md" :class="{ 'step-disabled': !grupoActual }">
              <div class="step-header row items-center q-mb-sm">
                <q-chip :color="grupoActual ? 'orange-8' : 'grey-4'"
                  :text-color="grupoActual ? 'white' : 'grey-6'"
                  size="sm" class="q-mr-sm">3</q-chip>
              ...
              <template v-if="grupoActual">
                <div class="row q-col-gutter-md">
                  ...
                  <div class="col-12 col-md-4">
                    <q-select v-model="form.dia" :options="diasOptions" label="Día *"
                      outlined dense emit-value map-options
                      :rules="[v => !!v || 'Selecciona un día']" />
                  </div>
                  <!-- Hora inicio -->
                  <div class="col-12 col-md-4">
                    <q-input v-model="form.hora_inicio" label="Hora Inicio *" type="time"
                      outlined dense :rules="[v => !!v || 'Campo obligatorio']" />
                  </div>
                  <!-- Hora fin -->
                  <div class="col-12 col-md-4">
                    <q-input v-model="form.hora_fin" label="Hora Fin *" type="time"
                      outlined dense :rules="[v => !!v || 'Campo obligatorio']" />
                  </div>
                  <!-- Aula -->
                  <div class="col-12 col-md-8">
                    <q-select v-model="form.aula_id" :options="aulasOptions"
                      label="Aula" outlined dense clearable
                      map-options emit-value option-value="id" option-label="label"
                      use-input input-debounce="200" @filter="filterAulas">
                  ...
                  <!-- ID API -->
                  <div class="col-12 col-md-4">
                    <q-input v-model="form.id_horario_api" label="ID API" type="number" outlined dense />
                  </div>
                </div>
              </template>
      ...
      <q-card>
        ...
        <q-card-section class="q-pt-lg">
          ...
          <div v-if="horarioToDelete" class="q-pa-sm bg-grey-2 rounded-borders">
            <div><strong>Asignatura:</strong> {{ horarioToDelete.grupo?.asignatura?.nombre || horarioToDelete.grupo?.asignatura_nombre || '—' }}</div>
            <div><strong>Grupo:</strong> {{ horarioToDelete.grupo?.nombre || horarioToDelete.grupo_id }}</div>
            <div><strong>Día / Hora:</strong> {{ formatDia(horarioToDelete.dia) }} {{ horarioToDelete.hora_inicio }} - {{ horarioToDelete.hora_fin }}</div>
          </div>
...
const form = ref({ id: null, grupo_id: null, aula_id: null, dia: '', hora_inicio: '', hora_fin: '', id_horario_api: null })
...
// ── Columnas tabla ──
const columns = [
  { name: 'sede',                  label: 'Sede',       field: r => r.grupo?.sede?.nombre   || r.grupo?.sede_nombre    || '', align: 'left',   sortable: true },
  { name: 'carrera',               label: 'Carrera',    field: r => r.grupo?.carrera?.nombre || r.grupo?.carrera_nombre || '', align: 'left',   sortable: true },
  { name: 'asignatura',            label: 'Asignatura', field: r => r.grupo?.asignatura?.nombre || r.grupo?.asignatura_nombre || '', align: 'left', sortable: true },
  { name: 'grupo',                 label: 'Grupo',      field: r => r.grupo?.nombre          || '', align: 'center', sortable: true },
  { name: 'aula',                  label: 'Aula',       field: r => r.aula?.nombre           || '', align: 'left',   sortable: true },
  { name: 'docente',               label: 'Docente',    field: r => docenteNombreHorario(r)  || '', align: 'left',   sortable: true },
  { name: 'dia',                   label: 'Día',        field: 'dia',                               align: 'center', sortable: true },
  { name: 'horario',               label: 'Horario',    field: r => `${r.hora_inicio} - ${r.hora_fin}`, align: 'center', sortable: true },
...
const uniqueAsignaturas = computed(() => new Set(filteredHorarios.value.map(h => h.grupo?.asignatura_id ?? h.grupo?.asignatura?.id).filter(Boolean)).size)
const modifiedCount     = computed(() => filteredHorarios.value.filter(h => h.modificado_localmente).length)
...
function docenteNombreHorario(h) {
  ...
  return d.nombre_completo || [d.nombre, d.apellido_paterno, d.apellido_materno].filter(Boolean).join(' ')
}
...
function formatDia(dia) {
  const m = { LUNES:'Lunes', MARTES:'Martes', MIERCOLES:'Miércoles', JUEVES:'Jueves', VIERNES:'Viernes', SABADO:'Sábado', DOMINGO:'Domingo' }
  return m[dia] || dia
}
function diaColor(dia) {
  const c = { LUNES:'blue-2', MARTES:'green-2', MIERCOLES:'amber-2', JUEVES:'orange-2', VIERNES:'red-2', SABADO:'purple-2', DOMINGO:'grey-3' }
  return c[dia] || 'grey-2'
}
function diaTextColor(dia) {
  const c = { LUNES:'blue-9', MARTES:'green-9', MIERCOLES:'amber-10', JUEVES:'orange-9', VIERNES:'red-9', SABADO:'purple-9', DOMINGO:'grey-8' }
  return c[dia] || 'grey-9'
}
...
function onSedeChange() { filterCarrera.value = null }
function clearFilters() { filterSede.value = null; filterCarrera.value = null; filterDia.value = null; filterTexto.value = '' }
...
async function openDialog(horario) {
  ...
  if (horario) {
    ...
    busqAsig.value         = ''
    resAsig.value          = []
    form.value = { id: null, grupo_id: null, aula_id: null, dia: '', hora_inicio: '', hora_fin: '', id_horario_api: null }
  }
...
// Eliminar
// ──────────────────────────────────────────────
function confirmDelete(horario) { horarioToDelete.value = horario; showDeleteDialog.value = true }
...
.stat-card    { background: var(--bg-secondary); border-color: var(--border-color); border-radius: 10px; transition: transform .15s; }
.stat-card:hover { transform: translateY(-2px); }
.horarios-table { border-radius: 12px; overflow: hidden; }
.horarios-table :deep(th) { font-weight: 600; font-size: .8rem; text-transform: uppercase; letter-spacing: .5px; color: var(--text-secondary); }
.horarios-table :deep(td) { font-size: .875rem; }
...
.step-header { border-bottom: 1px solid #e8e8e8; padding-bottom: 8px; margin-bottom: 12px !important; }
.resultado-list { max-height: 220px; overflow-y: auto; }
</style>
