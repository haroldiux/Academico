    ...
    <q-card class="table-card">
      ...
      >
        <template v-slot:no-data>
          <div class="full-width row flex-center text-accent q-gutter-sm" style="padding: 40px">
            ...
            <span v-if="!filtros.sede && !filtros.carrera">
              Seleccione una <strong>Sede</strong> o <strong>Carrera</strong> para ver más
              asignaturas.
            </span>
            <span v-else> No se encontraron asignaturas para esta selección. </span>
          </div>
        </template>
        ...
        <template v-slot:body-cell-docentes="props">
          <q-td :props="props">
            <div class="docentes-avatars">
              ...
              <span
                v-if="!props.row.docentes?.length"
        ...
        <template v-slot:body-cell-estado="props">
          <q-td :props="props">
            <q-badge
              :color="props.row.activa ? 'green-2' : 'grey-2'"
              :text-color="props.row.activa ? 'green-9' : 'grey-9'"
              class="q-px-sm"
              rounded
            >
        ...
        <template v-slot:body-cell-acciones="props">
          <q-td :props="props">
            ...
            <q-btn
              ...
              size="sm"
              color="green"
              @click="asignarDocentes(props.row)"
            >
            ...
            <q-btn
              ...
              size="sm"
              color="red"
              @click="eliminarAsignatura(props.row)"
            >
      ...
      <q-card class="dialog-card" style="min-width: 600px">
        ...
        <q-card-actions align="right" class="dialog-actions">
          ...
          <q-btn
            unelevated
            color="primary"
            :label="editMode ? 'Guardar Cambios' : 'Crear Asignatura'"
...
const columns = [
  ...
  { name: 'semestre', label: 'Semestre', field: 'semestre', align: 'center', sortable: true },
  {
    name: 'horas',
    label: 'Horas (T/P)',
    field: (row) => `${row.horas_teoricas}/${row.horas_practicas}`,
    align: 'center',
  },
  {
    name: 'carga_total',
...
const carrerasFiltradas = computed(() => {
  ...
  if (
    ...
    authStore.usuarioActual?.director?.carreras?.length > 0
  ) {
...
const carrerasFormOptions = computed(() => {
  ...
  return carrerasStore
    .getCarrerasBySede(form.value.sede_id)
    .map((c) => ({ label: c.nombre, value: c.id }))
})
...
function filterCarreras(val, update) {
  ...
  update(() => {
    ...
    carrerasOptions.value = carrerasFiltradas.value.filter(
...
const totalHoras = computed(() =>
  asignaturasFiltradas.value.reduce(
    (sum, a) => sum + (a.horas_teoricas || 0) + (a.horas_practicas || 0),
    0,
  ),
)
...
const docentesAsignados = computed(() => {
  ...
  asignaturasFiltradas.value.forEach((a) =>
    a.docentes?.forEach((d) => docentes.add(typeof d === 'object' ? d.id : d)),
  )
...
function openDialog(asignatura = null) {
  if (asignatura) {
    ...
    form.value = {
      ...
      sede_id: asignatura.carrera_id
        ? carrerasStore.getCarreraById(asignatura.carrera_id)?.sede_id
        : null,
    }
  } else {
...
async function guardarAsignatura() {
  try {
    if (!editMode.value && (!form.value.carrera_id || !form.value.semestre)) {
      $q.notify({
        type: 'negative',
        message: 'Carrera y semestre son obligatorios para crear una asignatura',
      })
      return
    }
    if (editMode.value) {
...
function asignarDocentes(asignatura) {
  ...
  // Pre-cargar solo IDs
  docentesSeleccionados.value = asignatura.docentes
    ? asignatura.docentes.map((d) => (typeof d === 'object' ? d.id : null)).filter(Boolean)
    : []
...
async function guardarDocentes() {
  if (asignaturaSeleccionada.value) {
    try {
      await asignaturasStore.assignDocentes(
        asignaturaSeleccionada.value.id,
...
function exportTable() {
  ...
  const data = asignaturasFiltradas.value.map((row) => {
    ...
    const sedeNombre =
      row.sede_nombre && row.sede_nombre !== 'N/A'
        ? row.sede_nombre
