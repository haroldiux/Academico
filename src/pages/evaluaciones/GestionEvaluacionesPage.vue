  ...
  <q-page class="gestion-eval-page">
    ...
    <div class="row q-mb-lg bg-white q-pa-sm rounded-borders shadow-1 items-center justify-center">
      ...
      <q-btn-toggle
        ...
        unelevated
        flat
        :options="[{ label: 'Todos', value: null }, ...estadosOptions]"
        no-caps
        class="custom-toggle"
      />
    </div>
          ...
          <q-td :props="props" align="center">
            <div class="flex items-center justify-center gap-1">
              ...
              <q-separator
                vertical
                class="q-mx-xs"
                v-if="props.row.patrones && props.row.patrones.length > 0"
              />
    ...
    try {
      ...
      if (examen.estado === 'entregados') {
        ...
        for (const letra of variants) {
          ...
          await api.post(`/rol-examenes/${examen.id}/upload-patron`, f1, {
            headers: { 'Content-Type': 'multipart/form-data' },
          })
          ...
          await api.post(`/rol-examenes/${examen.id}/upload-patron`, f2, {
            headers: { 'Content-Type': 'multipart/form-data' },
          })
        }
      }
    ...
    [
      {
        content: 'LOGO\nUNITEPC',
        rowSpan: 2,
        styles: { halign: 'center', valign: 'middle', fontSize: 8, cellWidth: 30 },
      },
      {
        content: 'UNIVERSIDAD TECNICA PRIVADA COSMOS\nGESTION I-2026',
        styles: { halign: 'center', fontSize: 12, fontStyle: 'bold', cellWidth: 120 },
      },
