      ...
      const mapped = jsonData
        .map((row, idx) => {
          ...
          return {
            ...
            respuesta_correcta: String(upperRow.RESPUESTA_CORRECTA || upperRow.RESPUESTA || '')
              .toUpperCase()
              .split(';')
              .map((r) => r.trim())
              .filter((r) => r),
...
const ejecutarGeneracionManual = async () => {
  ...
  try {
    ...
    const carreraObj = carrerasOptionsManual.value.find(
      (c) => c.value === manualConfig.value.carrera,
    )
    ...
    for (let i = 0; i < variantesLetters.length; i++) {
      ...
      const sorted = mezcladas.sort((a, b) => {
        ...
        let ta = a.tipo
          ?.toUpperCase()
          .replace('PR', 'PROBLEMA')
          .replace('EM', 'EMPAREJAMIENTO')
          .replace('SP', 'SUBPROBLEMA')
        let tb = b.tipo
          ?.toUpperCase()
          .replace('PR', 'PROBLEMA')
          .replace('EM', 'EMPAREJAMIENTO')
  ...
  if (examen.estado === 'programados') {
    ...
    try {
      ...
      if (data.success && data.configuracion) {
        ...
        const confParcial = data.configuracion.parciales.find(
          (p) =>
            ...
            (p.nombre.toLowerCase().includes('final') &&
              partialKey.toLowerCase().includes('final')) ||
            (p.nombre.toLowerCase().includes('instancia') &&
              partialKey.toLowerCase().includes('instancia')),
        )
...
const mezclarIncisos7167 = (preguntas) => {
  return preguntas.map((p) => {
    if (
      ['FALSO_VERDADERO', 'FV'].includes(p.tipo?.toUpperCase()) ||
      !p.opciones ||
      p.opciones.length === 0
    ) {
      return p
...
const generarExamenPDF = async (pdfDoc, examen, config, letra = 'A', preguntas = []) => {
  ...
  for (let i = 0; i < preguntas.length; i++) {
    ...
    let tipoActual =
      tipoActualOriginal === 'PR'
        ? 'PROBLEMA'
        : tipoActualOriginal === 'EM'
          ? 'EMPAREJAMIENTO'
          : ['SP', 'SUBPREGUNTA', 'SUBPROBLEMA'].includes(tipoActualOriginal)
            ? 'SUBPREGUNTA'
            : tipoActualOriginal
    ...
    if (tipoActual === 'EMPAREJAMIENTO') {
      ...
      while (
        j < preguntas.length &&
        (preguntas[j].tipo?.toUpperCase() === 'SUBPROBLEMA' ||
          preguntas[j].tipo?.toUpperCase() === 'SUBPREGUNTA') &&
